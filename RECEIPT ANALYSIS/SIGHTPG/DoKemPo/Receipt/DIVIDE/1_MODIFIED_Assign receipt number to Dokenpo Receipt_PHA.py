...
#import pandas_bj as pdbj

DIR = r"D:\土木健保データ原本写し\20191209\rece"
...
def files(folder_DIR):
    ...
    file_list = [o_name for o_name in open_name if (o_name.find('csv') > 0) | (o_name.find('CSV') > 0)]
...
for i, folder_DIR in enumerate(folders_DIR):
    ...
    if len(folder_list(folder_DIR)) == 1:
        ...
        dum = [os.path.join(os.path.join(folder_DIR, folder_list(folder_DIR)[0]), f) for f in fs_list]
        FILE_PATH_dic[i] = dum
        ...
        dum = [os.path.join(folder_DIR,  f) for f in files(folder_DIR)]
        FILE_PATH_dic[i] = dum
...
def fdic(folder):
    fnames = files(folder)
    ftuple = [(fname, fname.split('.')[0].split('_')[2].lower()) for fname in fnames]
    return {k: v for (k, v) in ftuple}
...
def get_ptype(folder_DIR):
    ...
    for item in fdict:
        if TARGET_TYPE.upper() in item:
            file_name = item
        if TARGET_TYPE.lower() in item:
            file_name = item
...
def divide_prescriptions(df, type_):
    ...
    if type_.lower()   == 'dpc':
        prescriptions = {dpc.lower(): df[df[3] == dpc.upper()] for dpc in dpc_list}
    elif type_.lower() == 'med':
        prescriptions = {med.lower(): df[df[3] == med.upper()] for med in med_list}
    elif type_.lower() == 'pha':
        prescriptions = {pha.lower(): df[df[3] == pha.upper()] for pha in pha_list}
        pha_df = df
        
    prescriptions = valmap(lambda d: d.dropna(how='all', axis=1), prescriptions)
    del df
    
    if type_.lower() == 'pha':
        # shcziy fileを作成
        shcziy = pha_df[pha_df[3] == 'SH']
        shcziy = shcziy.append(pha_df[pha_df[3] == 'CZ'])
        shcziy = shcziy.append(pha_df[pha_df[3] == 'IY'])
        # sh fileを作成
        sh = pha_df[pha_df[3] == 'SH'] 
        sh = sh['index']
        ...
        for i in range(len(sh_list)):
            shcziy_dic[i] = pd.merge(left = shcziy_dic[i], right = sh_dic[i], 
                                        left_on=['index'], right_on=['sh_num'], 
                                        how='left', sort = True)
...
def writing(prescriptions, ptype, type_DIR, type_list, cym):
    for rsname_l in type_list:
        #try:
            ...
            prescriptions[rsname].to_csv(dlname)
...
for folder_DIR in folders_DIR:
    ...
    file_name = get_ptype(folder_DIR)[0]
    ptype = get_ptype(folder_DIR)[1]
    
    df = assign_rnum(file_name)
    
    prescriptions = divide_prescriptions(df, ptype)
    naming(prescriptions, ptype)
